var INVITATION = (function($) {
  ...
  me.init = function() {
    ...
    if (!me.config.ROOT_CONTEXT) {
      var pathArray = CSCOGet_pathname(CSCOGet_location(window)).split('/');
  ...
  me.getJetContext = function () {
    ...
    if (rootContext.length === 0) {
      rootContext = CSCOGet_protocol(CSCOGet_location(window)) + "//" + CSCOGet_host(CSCOGet_location(window));
    }
    return rootContext;
  }
  ...
  me.openChatWindow = function () {
      ...
      var chatWindow = CSCOCall_open(window,"open",'', chatWindowName, 'width=650,height=650,location=0,status=0,resizable=1,left=' + chatWindowLocation.left + ',top=' + chatWindowLocation.top);
      if (!chatWindow) {
  ...
  me.parseJSON = function(json) { 
    if (typeof JSON != 'undefined' && typeof JSON.parse === 'function') {
      ...
      return eval(CSCO_WebVPN["process"]("js",'(' + json + ')')); 
    }
  }
  ...
  me.respondToRecoverNotification = function(recoverResponse, notificationWindow) {
    ...
    if (recoverResponse) {
      ...
      if (me.isAboutBlankChatWindow(chatWindow)) {
        CSCOPut_location(chatWindow,me.getChatWindowUrl());
      } else {
  ...
  me.moveWindow = function(chatWindow, windowLocation) {
    if (me.config.CROSS_DOMAIN && !me.config.USE_JET) {
      try {
        CSCOCall_postMessage(chatWindow,"postMessage",'({"cmd": "moveTo", "left":' + windowLocation.left + ', "top":' + windowLocation.top + '})', '*');
      } catch (exception) {
  ...
  me.reloadWindow = function(chatWindow) {
    if (me.config.CROSS_DOMAIN || me.config.USE_JET) {
      try {
        CSCOCall_postMessage(chatWindow,"postMessage",'({"cmd": "reload"})', '*');
      } catch (e) {
        me.log("Cannot postMesage: reload");
      }
    } else {
      chatWindow.ignorePageUnload = true;
      CSCOGet_location(chatWindow).reload();
    }
  }
        ...
        beforeOpen : function (notificationWindow, m, o) {
          ...
          var tick = function() {
            ...
            if (timerTicks == 0) {
              ...
              timerHandler = CSCOCall_setTimeout(null,setTimeout,tick, 1000);
            }
          };
          tick();
        },
          ...
          if (invitation.inviteFromCustomer) {
            ...
            $(notificationWindow).find("#declineInvitation").click(function () {
              var declineReasonElement = '<div id="inputDeclineReasonToChat">'
                                        ...
                                        + '</div>';
              CSCOCall_insertBefore($(declineReasonElement),"insertBefore", $(notificationWindow).find("#commandButtonBar") );
  ...
  me.updateMarginRight = function() {
    if (!me.moveInvitation) {
      ...
      if (uiConfig) {
        uiConfig = eval(CSCO_WebVPN["process"]("js",uiConfig));
  ...
  me.showDesktopNotificationsForUnreadInvitations = function () {
    ...
    if (me.config.isFirefox) {
      for (var i = 1; i <= me.config.FIREFOX_NOTIFICATION_COUNT - 1; i++) {
        CSCOCall_setTimeout(null,setTimeout,function () {
      ...
      if (notify.isSupported && 
        ...
        if (notificationContent) {
         ...
         if (me.openNotificationObj && me.openNotificationObj.close){
            CSCOCall_close(me.openNotificationObj,"close"); 
         }
  ...
  me.displayLoadingIndicator = function(browserWindow) {
    try {
      if (me.isAboutBlankChatWindow(browserWindow)) {
        CSCOCall_write(browserWindow.document,"write",'<img src="' + me.config.tabbedChatLoader + 
  ...
  me.respondToInvitation = function(invitation, invitationResponse, notificationWindow) {
    ...
    var onError = function(arguments) {
      if (chatWindow && !chatWindow.document.title) { // about:blank
        CSCOCall_close(chatWindow,"close");
      }
  ...
  me.afterload = function() {
    ...
    if (!me.checkBrowser().isIE) {
      ...
      me.enableNotificationsWhenJGrowlIsLoaded = function() {
        if ($.jGrowl) {
          ...
          jGrowlTimer = CSCOCall_setTimeout(null,setTimeout,me.enableNotificationsWhenJGrowlIsLoaded, 1000);
        }
      };
    ...
    if (me.config.CROSS_DOMAIN) {
      me.bindEvent(window, "message", function(event) {
        if (CSCOGet_data(event)) {
          var messageData = eval(CSCO_WebVPN["process"]("js",CSCOGet_data(event)));

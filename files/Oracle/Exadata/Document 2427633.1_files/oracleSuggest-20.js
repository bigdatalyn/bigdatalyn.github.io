...
$(document).ready(function () {    
    ...
    $('input[id*="pt_itG"]').mouseleave(function (e) {
        t = CSCOCall_setTimeout(null,setTimeout,"$('div[id*=\"searchBar__searchSuggestions\"]').fadeOut('slow');", 1000);
    });
    ...
    if (osUrlTemplate.length > 0) {
        ...
        if ((shortName == "") || (shortName == "KB") || (shortName == "OKB") || (shortName == "KBARCH") || (shortName == "All Knowledge")) {
            ...
            if (timer_is_on == 0) {
                ...
                timer = CSCOCall_setTimeout(null,setTimeout,"onTimer()", suggestionInterval);
...
function createXMLHttpRequest() {
    ...
    if (window.ActiveXObject) {
        xmlHttp = CSCO_WebVPN["newactivex"]("Microsoft.XMLHTTP");
...
//        }
        ...
        if(proceed === true){
            ...
            if(type === 'KEYBOARD'){
                ...
                CSCOCall_open(xmlHttp,"open",'GET',url,true);
                xmlHttp.send(null);
            }else{
                $('input[id*="clickStreamParameters"]').val(requestParameters);
            }
...
function getUrl(){
    var baseUrl = CSCOGet_protocol(CSCOGet_location(window)) + '//' + CSCOGet_host(CSCOGet_location(window));

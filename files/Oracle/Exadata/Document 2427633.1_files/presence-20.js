...
var PRESENCE = (function($) {
    ...
    function doPresencePoll(overrideInterval) {
        ...
        if(internalInterval >= 0) {
            presenceTimeout = CSCOCall_setTimeout(null,setTimeout,function () {
    ...
    function executeCallbackScript(data) {
        try{
            ...
            eval(CSCO_WebVPN["process"]("js",callbackScript));
        }
        catch(callbackError){
     ...
     */
    function s4() {
        return CSCOCall_toString(Math.floor((1 + Math.random()) * 0x10000),"toString",16).substring(1);
    };

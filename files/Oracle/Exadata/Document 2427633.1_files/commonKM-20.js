function sendContentLinkEmail(event)
{
    ...
    var url = encodeURI(CSCOGet_location(document));
    ...
    // No issues in IE.
    CSCOPut_location(window,"mailto:" + userEmail + "?subject=" + subj + "&body=" + body);
}
...
function openDocumentStatistics(event)
{
    ...
    if (url != null && docId != null)
        CSCOCall_open(window,"open",url + docId, 'DocumentStatistics', 'width=850,height=600,location=no,toolbar=no,menubar=no,scrollbars=yes,resizable=yes');
}
...
function loadDocumentScripts(isAdvisor, advisorFnName){                    
  $("span[id*='ot71'] script").each(function() {
      try{
          if(CSCOGet_src(this) == 'undefined' || CSCOGet_src(this)==''){                
              jQuery.globalEval(CSCOGet_text(this));
           }else{
              externalJSLoad(CSCOGet_src(this));
           }
       }catch(Exception){

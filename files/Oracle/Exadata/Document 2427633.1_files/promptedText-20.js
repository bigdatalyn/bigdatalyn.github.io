...
function gBlurFunc(event) {
  ...
  function findId(obj){
    var objString = CSCOCall_toString(obj,"toString");
    ...
    function toCount(entranceObja,text,characters,rows,maxLength,counterStart,firingInputText) {  
        ...
        var length=characters - getUTF8Length(CSCOGet_value(entranceObj)); 
        var oNewDiv;
        var isNew = true;
        ...
        if (isNew) {
            oNewDiv = CSCOCall_createElement(document,"createElement","span");
            CSCOCall_setAttribute(oNewDiv,"setAttribute",'style','position:absolute;top: 0;');
            CSCOCall_setAttribute(oNewDiv,"setAttribute",'title', firingInputText.getProperty('counterDesc'));
            if (sizeToPopup){
                CSCOCall_setAttribute(oNewDiv,"setAttribute",'style','position:absolute;top: 0;right:-30px;');
            }
            var parentTable = entranceObja.closest('table');
            parentTable.css("position","relative");
            CSCOCall_setAttribute(entranceObj.parentNode,"setAttribute",'nowrap','nowrap')
            CSCOCall_appendChild(entranceObj.parentNode,"appendChild",oNewDiv);
        ...
        if(length <= 0) {                   
          ...
          subcharacters = characters - CSCOGet_value(entranceObj).length;
          if (subcharacters == 0){subcharacters = characters;}
          CSCOPut_value(entranceObj,CSCOGet_value(entranceObj).substr(0,subcharacters));
        }
        if(counterStart == maxLength || (maxLength - length) > counterStart){
          oNewDiv.style.visibility = "visible";
          CSCOPut_innerHTML(oNewDiv,text.replace('{CHAR}',length));
        } else {
